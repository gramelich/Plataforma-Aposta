...
class DoubleController extends Controller {
 ...
 de aposta é de R$ '.$this->settings->wheel_min_bet.'!']);
		...
		try {
			$bet = new DoubleBets();
			if($this->user->is_admin == 1) $bet->fake = 1;
            if($this->user->is_youtuber == 1) $bet->fake = 1;
            if($this->user->is_moder == 1) $bet->fake = 1;
			...
			if($balance == 'balance') {
                ...
                if($this->user['ref_id'] != null) {
					$user = User::where('unique_id', $this->user['ref_id'])->first();
					if($user) {
						$reward = ($price*$this->user['ref_revenue'])/100;
						$user->ref_money_all += $reward;
						$user->ref_money += $reward;
						$user->save();
 ...
 de aposta é de R$ '.$this->settings->wheel_min_bet.'!']);
	...
	private function multiplierDouble($color) {
	...
	public function getSlider(Request $r) {
		...
		if($checkUser >= 1 && $this->game->ranked != 1) {
			$minProfit = $this->settings->profit_money;
			if($minProfit <= 0) $minProfit = 2;
			$minProfit = $minProfit / 2;

			if((($this->settings->profit_money - ($prices[$wcolor] * $this->multiplierDouble($wcolor))) <= $minProfit) || $profit_wheel - ($prices[$wcolor] * $this->multiplierDouble($wcolor)) <= $minProfit) {
				$wcolor = 'red';
				if($prices['black'] * 2 <= $prices['red'] * 2 && $prices['black'] * 2 <= $prices['white'] * 14) $wcolor = 'black';
				if($prices['red'] * 2 <= $prices['black'] * 2 && $prices['red'] * 2 <= $prices['white'] * 14) $wcolor = 'red';
				if($prices['white'] * 14 <= $prices['black'] * 2 && $prices['white'] * 14 <= $prices['red'] * 2) $wcolor = 'white';
				if($wcolor == 'white' && $prices['white'] * 14 == $prices['black'] * 2) $wcolor = 'black';
				if($wcolor == 'white' && $prices['white'] * 14 == $prices['red'] * 2) $wcolor = 'red';
	...
	private function sendMoney($game_id, $color) {
		...
		$multiplier = ($color == 'black') ? 2 : (($color == 'red') ? 2 : (($color == 'white') ? 14 : 1));
		$profit = DoubleBets::where(['game_id' => $game_id, 'fake' => 0])->sum('price');
        foreach($bets as $b) {
            $user = User::where(['id' => $b->user_id, 'fake' => 0])->first();
            if(!is_null($user) && !$user->is_admin && !$user->is_youtuber) {
			...
			if($user && $user['ref_id'] != NULL) {
				$refUser = User::where('unique_id', $user['ref_id'])->first();
				if($refUser) {
					$reward = ($b->price*$refUser['ref_revenue'])/100;
					$refUser->ref_money_all -= $reward;
					$refUser->ref_money -= $reward;
					$refUser->save();
