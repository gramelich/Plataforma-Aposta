...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        $profit_double = Profit::where('game', 'double')->where('created_at', '>=', Carbon::today())->sum('sum');
        ...
        return view('admin.index', compact('pay_today', 'pay_week', 'pay_month', 'pay_all', 'with_req', 'with_today', 'usersCount', 'profit_jackpot', 'profit_pvp', 'profit_battle', 'profit_wheel', 'profit_dice', 'profit_double', 'profit_crash', 'profit_hilo', 'profit_exchange', 'profit', 'profit_ref', 'fake', 'last_dep', 'users', 'userTop'));
    }
    ...
    public function userSave(Request $r)
    {
        ...
        User::where('id', $r->get('id'))->update([
            ...
            'ref_revenue' => $r->get('ref_revenue'),
            'ref_cpa' => $r->get('ref_cpa'),
            'ban' => $r->get('ban'),
    ...
    public function withdrawSend($id) {
        ...
        $data_string = json_encode(
            array(
                  ...
                  "description"=> "Pagamento EzBank",
                  "creditParty"=>array(
                      "key"=> $withdraw->wallet,
                      "keyType"=> "CPF",
                      "taxId"=> $withdraw->wallet,
                      "name"=> "Payment Proof"
                  ),
                  "urlNotificacao"=> "http://url-webhook-pagamento/"
            )
        );
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://api.ezzebank.com/v1/pix/payment',
            ...
            'Authentication: JDJ5JDEwJDkucTAvTXhZUFVzVmkuR1ZuNlpkLi5IZ1dnMkhuV2FpVm9HR21zcTlhbExZVzdBV1Nya0pT',
        ...
        $error = false;
        if (curl_error($curl)) $error = true;
        $response = json_decode($response, true);

        if ($error) return redirect()->route('admin.withdraws')->with('error', 'Erro interno, tente novamente.');
