...
class CrashController extends Controller
{
        ...
        try {
            ...
            if ($balType == 'balance') {
                ...
                if($this->user['ref_id'] != null) {
					...
					if($user) {
						$reward = (floatval($r->get('bet'))*$this->user['ref_revenue'])/100;
            ...
            if($this->user->is_admin == 1) $fake_user = 1;
            if($this->user->is_youtuber == 1) $fake_user = 1;
            if($this->user->is_moder == 1) $fake_user = 1;
    ...
    public function getFloat()
    {
        ...
        if($profitMin <= 0) $profitMin = 1;
        $profitMin = $profitMin / 2;

        if (($this->settings->profit_money - ($betsPrice * 2)) <= $profitMin && $betsPrice > 0) return round('1.' . mt_rand(0, 1) . mt_rand(0, 5), 2);
        if (($profit_crash - ($betsPrice * 2)) <= 0 && $betsPrice > 0) return round('1.' . mt_rand(0, 1) . mt_rand(0, 5), 2);
    ...
    public function Cashout()
    {
        ...
        try {
            ...
            if($this->user && $this->user['ref_id'] != NULL) {
				$refUser = User::where('unique_id', $this->user['ref_id'])->first();
				if($refUser) {
					$reward = (round($bet->price * $float, 2)*$refUser['ref_revenue'])/100;
        ...
        try {
            foreach ($bets as $bet) {
                ...
                if (!is_null($user) && $bet->withdraw < $this->game->multiplier) {
                    ...
                    $total -= round($bet->price * $bet->withdraw, 2);

                    if($user && $user['ref_id'] != NULL) {
                        $refUser = User::where('unique_id', $user['ref_id'])->first();
                        if($refUser) {
                            $reward = (round($bet->price * $bet->withdraw, 2)*$refUser['ref_revenue'])/100;
                            $refUser->ref_money_all -= $reward;
                            $refUser->ref_money -= $reward;
                            $refUser->save();
                    ...
                    //     }
                    // }
                } else if(!is_null($user) && $bet->withdraw >= $this->game->multiplier) {
                    $total += round($bet->price, 2);
                }
            }
            DB::commit();
        } catch (Exception $e) {
