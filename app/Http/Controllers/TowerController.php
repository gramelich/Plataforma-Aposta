...
use App\Payments;
use App\Profit;
use App\Services\AffiliatedService;
...
class TowerController extends Controller
{
    ...
    public function newGame(Request $r)
    {
        ...
        if (
            Payments::where('user_id', $this->user->id)->where('status', 1)->count() > 0 &&
            $this->user->balance == 0 &&
            $this->user->requery >= ($this->user->bonus*50)
        ) {
            $this->user->balance = $this->user->bonus;
            $this->user->bonus = 0;
            $this->user->requery = 0;
            $this->user->save();
        } elseif($this->user->balance == 0) {
            $balType = 'bonus';
        }
        ...
        if (!is_null($this->user->ref_id)) {
            $affiliatedProfit = $bet * 0.63;
            AffiliatedService::increaseRefMoney($this->user->ref_id, $affiliatedProfit);
        }
    ...
    public function next(Request $r)
    {
		...
		$profit_tower = Profit::where('game', 'tower')->where('created_at', '>=', Carbon::today())->sum('sum');

		$coeffProfit = round(self::COEFF[$game->bombs][count($revealed) + 1 - 1] - self::COEFF[$game->bombs][count($revealed) + 1 - 1] * 5 / 100, 2);
		$winSumProfit = round($game->bet * $coeffProfit, 2);
		$profitMin = $this->settings->profit_money;
		if($profitMin <= 0) $profitMin = 2;
		$profitMin = $profitMin / 2;

		if($this->settings->profit_money - $winSumProfit <= $profitMin && $profit_tower <= 0) {
			if($this->user->is_admin != 1 && $this->user->is_youtuber != 1 && $this->user->is_moder != 1) {
				$profitWin = false;
			}
		}

		if ($rand < self::CHANCES[$game->bombs] && $profitWin) {
			$slots = [0, 1, 2, 3, 4];
			unset($slots[$slot]);
			$revealed[] = $slot;
			$fiels = [];
			...
			if($this->user->is_admin != 1 && $this->user->is_youtuber != 1 && $this->user->is_moder != 1) {
				...
				$this->settings->profit_money += round($game->bet, 2);
				$this->settings->save();
			}
		}
		...
		if (count($revealed) === 10) {
			...
			if($this->user->is_admin != 1 && $this->user->is_youtuber != 1 && $this->user->is_moder != 1) {
				...
				$this->settings->profit_money -= round($winSum - $game->bet, 2);
				$this->settings->save();
			}
			...
			if ($this->user['ref_id'] != NULL) {
				...
				$userRef->ref_money_all -= $winSum * ($this->user->revenue_share_percent / 100); // 10 BRL
				$userRef->ref_money -= $winSum * ($this->user->revenue_share_percent / 100); // 10 BRL
				$userRef->save();

				Profit::create([
					'game' => 'ref',
					'sum' => -($winSum * ($this->user->revenue_share_percent / 100))
				]);
			...
			if ($balType === 'bonus') {
				$this->redis->publish('updateBonus', json_encode([
                    ...
                    'wallet' => round($this->user->totalWallet(), 2)
				]));
			}
		}
    ...
    public function claim()
    {
        ...
        // if (!is_null($this->user->ref_id)) {
        //     $refLoss = $game->bet * 0.63;
        //     $refLoss += round($winSum-$game->bet, 2) * 0.63;
        //     AffiliatedService::decreaseRefMoney($this->user->ref_id, $refLoss);
        // }
		...
		// 	$userRef->ref_money_all -= $winSum * ($this->settings->revenue_share_percent / 100); // 10 BRL
		// 	$userRef->ref_money -= $winSum * ($this->settings->revenue_share_percent / 100); // 10 BRL
		// 	$userRef->save();
